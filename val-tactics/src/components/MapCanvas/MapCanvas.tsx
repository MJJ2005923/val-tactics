...
export default function MapCanvas({ mapId, mapName: _mapName, transformRef }: MapCanvasProps) {
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (data.type === 'agent') {
      ...
      if (shapeConfig) {
        ...
        } else if ((shapeConfig.shape === 'line' && data.abilityId !== 'harbor-reckoning' && data.abilityId !== 'sova-hunters-fury') || data.abilityId === 'miks-x' || data.abilityId === 'tejo-q' || data.abilityId === 'phoenix-curveball' || data.abilityId === 'neon-fast-lane' || data.abilityId === 'neon-high-gear') {
  ...
  const handleCanvasClick = useCallback((e: React.MouseEvent) => {
    ...
    if (lineDrawing && lineDrawing.mode !== 'freehand') {
      ...
      if (lineDrawing.abilityId === 'neon-fast-lane' || lineDrawing.abilityId === 'neon-high-gear') {
      ...
      {lineDrawing && (() => {
        ...
        return (
          <svg style={{ position: 'absolute', inset: 0, zIndex: 5, pointerEvents: 'none', overflow: 'visible' }}>
            ...
            {/* 直线模式预览 */}
            {!isFreehand && (() => {
              const isNeonC = lineDrawing.abilityId === 'neon-fast-lane' || lineDrawing.abilityId === 'neon-high-gear'
